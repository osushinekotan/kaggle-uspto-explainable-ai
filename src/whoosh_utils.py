...
class QueryValidator:
    def validate_query(self, query, parent_has_index=False):  # noqa
        index_allowlist = "|".join(["ab", "clm", "cpc", "detd", "ti"])
        index_regexp = re.compile(r"\.(" + index_allowlist + r")\.$")
...
class BrsPhrase(whoosh.query.Phrase):
    ...
    def normalize(self):
        ...
        return self.__class__(
            self.fieldname, words, slop=self.slop, boost=self.boost, char_ranges=self.char_ranges, ordered=self.ordered
        )
...
class BrsProximityPlugin(whoosh.qparser.Plugin):
    ...
    class BrsProximityNode(whoosh.qparser.TextNode):
        ...
        def apply(self, fn):
            return self.__class__(
                self.type, [fn(node) for node in self.nodes], slop=self.slop, boost=self.boost, ordered=self.ordered
            )
        ...
        def query(self, parser):
            ...
            qclass = BrsPhrase
            q = qclass(
                fieldname, words, slop=self.slop, boost=self.boost, char_ranges=char_ranges, ordered=self.ordered
            )
...
def count_query_tokens(query: str):
    ...
    return len([i for i in re.split(r"[\s+()]", query) if i])
